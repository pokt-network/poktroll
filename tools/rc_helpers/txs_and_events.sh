...
# find . -name "*.proto" -exec grep -h "^message Event" {} \; | sed 's/^message \(Event[^{]*\).*/\1/'
...
function help() {
  ...
  echo "  shannon_query_unique_tx_msgs_and_events --help"
  echo "  shannon_query_tx_messages --help"
  echo "  shannon_query_tx_events --help"
  echo "  shannon_query_block_events --help"
  echo ""
  echo "Examples"
  ...
  echo "  shannon_query_tx_messages $LATEST_BLOCK_MINUS_100 $LATEST_BLOCK /pocket.supplier.MsgUnstakeSupplier pokt1abc123... main"
...
function shannon_query_unique_tx_msgs_and_events() {
  ...
  local TMP_FILE="/tmp/shannon_txs_${START_HEIGHT}_${END_HEIGHT}_${ENV}.json"

  # Check if file already exists and skip query if it does
  if [[ -f "$TMP_FILE" ]]; then
    echo "Using existing cached data from $TMP_FILE"
  else
...
# -----------------------------------------------
# shannon_query_unique_block_events
# -----------------------------------------------
# Description:
...
#   $3 - Network environment - must be one of: alpha, beta, main
#   $4 - (Optional) Event type prefix filter (e.g., "pocket" for pocket events only)
#   $5 - (Optional) --include-ignored - if provided, includes default ignored event types
#   $6 - (Optional) --show-count - if provided, shows count of each unique event type
#
# Example:
...
function shannon_query_unique_block_events() {
  if [[ "$1" == "--help" || "$1" == "-h" ]]; then
    echo "shannon_query_unique_block_events - Query unique block events from block results"
    echo ""
    echo "DESCRIPTION:"
    echo "  Queries block results for a range of block heights and returns only unique event types"
    echo "  and their attributes, removing duplicates and providing a summary of all distinct"
    echo "  events found in the block range."
    echo ""
    echo "USAGE:"
    echo "  shannon_query_unique_block_events <start_height> <end_height> <env> [event_prefix] [--include-ignored] [--show-count]"
    echo ""
    echo "ARGUMENTS:"
    echo "  start_height    Start block height (inclusive)"
    echo "  end_height      End block height (inclusive)"
    echo "  env             Network environment - must be one of: alpha, beta, main"
    echo "  event_prefix    (Optional) Event type prefix filter (e.g., 'pocket' for pocket events)"
    echo "  --include-ignored (Optional) Include default ignored event types"
    echo "  --show-count    (Optional) Show count of each unique event type"
    echo ""
    ...
    echo ""
    echo "EXAMPLES:"
    echo "  shannon_query_unique_block_events 115575 115580 main"
    echo "  shannon_query_unique_block_events 115575 115580 main pocket"
    echo "  shannon_query_unique_block_events 115575 115580 main \"\" --include-ignored --show-count"
    return 0
  fi
  ...
  if [[ $# -lt 3 ]]; then
    ...
    echo ""
    echo "Usage:"
    echo "  shannon_query_unique_block_events <start_height> <end_height> <env (alpha|beta|main)> [event_prefix] [--include-ignored] [--show-count]"
    echo ""
    echo "Example:"
    echo "  shannon_query_unique_block_events 115575 115580 main"
    echo "  shannon_query_unique_block_events 115575 115580 main pocket"
  ...
  if [[ -n "$EVENT_PREFIX" ]]; then
    echo "Filtering for events starting with: $EVENT_PREFIX"
  fi
  ...
  for ((height = $START; height <= $END; height++)); do
    ...
    echo "Processing block $height... to $BLOCK_TMP"
    ...
    # Process the block data and collect events
    jq --argjson height "$height" \
      ...
      --arg include_ignored "$INCLUDE_IGNORED" '
          ...
          def process_events(events; source):
            events
            | map(select(
                # If event_prefix is specified, only include events starting with that prefix
  ...
  # Process collected events to find unique ones
  echo ""
  ...
  if [[ "$SHOW_COUNT" == "true" ]]; then
    # Show summary first, then detailed info
    jq -r '
      if type == "string" then
        .
      else
  ...
  echo "Processed results saved to: $TMP_FILE"
...
function shannon_query_block_events() {
  ...
  for ((height = $START; height <= $END; height++)); do
    ...
    mkdir -p /tmp/blocks
    local BLOCK_TMP="/tmp/blocks/$height.json"

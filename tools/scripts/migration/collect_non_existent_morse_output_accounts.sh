...
if [ "$PRINT_COUNTS" = true ]; then
  echo "Total Non-Custodial Morse Accounts: $(echo "$ALL_MORSE_OUTPUT_ADDRESSES" | count_non_empty_lines)"
  echo "Total Morse claimable accounts: $(echo "$ALL_MORSE_CLAIMABLE_ACCOUNT_SRC_ADDRESSES" | count_non_empty_lines)"
  echo "Total missing MorseClaimableAccounts: $(echo "$MISSING_MORSE_ACCOUNT_ADDRESSES" | count_non_empty_lines)"
  exit 0
fi

...
func (s *MigrationModuleTestSuite) TestClaimMorseMultiSigAccount() {
	...
	morseSrcAddr, claimAccountRes := s.ClaimMorseMultiSigAccount(s.T(), 1, shannonDestAddr, sample.AccAddress())

	expectedMorseClaimableAccount := s.GetAccountState(s.T()).Accounts[1]

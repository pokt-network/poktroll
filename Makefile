...
send_relay_sovereign_app_JSONRPC: warn_message_acc_initialize_pubkeys # Send a JSONRPC relay through the AppGateServer as a sovereign application
	curl -X POST -H "Content-Type: application/json" \
	--data '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' \
	$(APPGATE_SERVER)/anvil

.PHONY: send_relay_delegating_app_JSONRPC
send_relay_delegating_app_JSONRPC: warn_message_acc_initialize_pubkeys # Send a relay through the gateway as an application that's delegating to this gateway
...
send_relay_sovereign_app_REST: warn_message_acc_initialize_pubkeys # Send a REST relay through the AppGateServer as a sovereign application
	curl -X POST -H "Content-Type: application/json" \

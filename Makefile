...
localnet_regenesis: ## Regenerate the localnet genesis file
	...
	jq --argjson pubKey "$$PUB_KEY" '. + {"validators": [{"address": "'$$ADDRESS'", "pub_key": $$pubKey, "power": "900", "name": "Rollkit Sequencer"}]}' ${HOME}/.poktroll/config/genesis.json > temp.json && mv temp.json ${HOME}/.poktroll/config/genesis.json; \

...
gateway_stake: ## Stake tokens for the gateway specified (must specify the gateway env var)
	pocketd --home=$(POCKETD_HOME) tx gateway stake-gateway -y --config $(POCKETD_HOME)/config/$(STAKE) --keyring-backend test --from $(GATEWAY) --node $(POCKET_NODE)
...
gateway_unstake: ## Unstake an gateway (must specify the GATEWAY env var)
	pocketd --home=$(POCKETD_HOME) tx gateway unstake-gateway -y --keyring-backend test --from $(GATEWAY) --node $(POCKET_NODE)

...
telegram_test_broadcast_msg: ## Test broadcast message from file. Usage: make telegram_test_broadcast_msg MSG_FILE=message.txt
	@if [ -z "$(MSG_FILE)" ]; then \
		echo "Error: MSG_FILE parameter is required. Usage: make telegram_test_broadcast_msg MSG_FILE=message.txt"; \
		exit 1; \
	fi

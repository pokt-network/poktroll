...
type TestSuite struct {
	...
	// applicationKeeper *mocks.MockApplicationKeeper
	// proofKeeper       *mocks.MockProofKeeper

	encCfg moduletestutil.TestEncodingConfig
}

// groupID uint64
// roupPolicyAddr sdk.AccAddress
...
func (s *TestSuite) SetupTest() {
	...
	// s.encCfg = moduletestutil.MakeTestEncodingConfig(tokenomics.AppModuleBasic{})
	...
	// testCtx := testutil.DefaultContextWithDB(s.T(), key, storetypes.NewTransientStoreKey("transient_test"))
	// encCfg := moduletestutil.MakeTestEncodingConfig(tokenomics.AppModuleBasic{}, bank.AppModuleBasic{})
	...
	// // Mock the BankKeeper
	// s.bankKeeper = mocks.NewMockBankKeeper(ctrl)

	// // Mock the AccountKeeper
	// s.accountKeeper = mocks.NewMockAccountKeeper(ctrl)
	// for i := range s.addrs {
	...
	// // Mock the ApplicationKeeper
	// s.applicationKeeper = mocks.NewMockApplicationKeeper(ctrl)

	// // Mock the ProofKeeper
	// s.proofKeeper = mocks.NewMockProofKeeper(ctrl)
	...
	// // 	s.encCfg.TxConfig,
	// // )
	// // bApp.SetInterfaceRegistry(s.encCfg.InterfaceRegistry)
	// // banktypes.RegisterMsgServer(bApp.MsgServiceRouter(), s.bankKeeper)

	// // config := tokenomics.DefaultConfig()
	// s.tokenomicsKeeper = keeper.NewKeeper(
	// 	s.encCfg.Codec,
	// 	storeService,
	// 	log.NewNopLogger(),
	// 	"authority",
...
func (s *TestSuite) TestSettleClaims() {
	...
	// t := s.T()
	// keepers, ctx := keepertest.NewTokenomicsModuleKeepers(t)
	// srv := keeper.NewMsgServerImpl(*keepers.Keeper)

	// keepers.service := &sharedtypes.Service{Id: testServiceId}
	...
	s.keepers.SettleExpiringClaims(sdkCtx)
	claims = s.keepers.GetAllClaims(ctx)
	s.Require().Len(claims, 1)
	...
	// 	"",
	// )
	// srv.createClaimRes, err := srv.CreateClaim(ctx, claimMsg)
	// Test when there are no claims to settle
	// Check that num claims goes down
	// Check that rewards are distributed
	// Check what claim didn't need a proof
	// Check when claim did need a proof

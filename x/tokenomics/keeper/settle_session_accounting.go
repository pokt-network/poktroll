...
func (k Keeper) SettleSessionAccounting(
	...
	if application.GetStake().IsLT(settlementCoin) {
		...
		applicationOverservicedEvent := &tokenomics.EventApplicationOverserviced{
		...
		if err = eventManager.EmitTypedEvent(applicationOverservicedEvent); err != nil {
			return tokenomics.ErrTokenomicsApplicationOverserviced.Wrapf(
	...
	if err = k.bankKeeper.BurnCoins(
		...
		return tokenomics.ErrTokenomicsApplicationModuleBurn.Wrapf("burning %s from the application module account: %v", settlementCoin, err)
	}

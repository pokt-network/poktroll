...
func (k Keeper) SettlePendingClaims(ctx sdk.Context) (
	settledResult tokenomics.PendingClaimsResult,
	expiredResult tokenomics.PendingClaimsResult,
	err error,
) {
	...
	settledResult = tokenomics.NewClaimSettlementResult()
	expiredResult = tokenomics.NewClaimSettlementResult()
	...
	for _, claim := range expiringClaims {
		var (
			...
			proofRequirement       proof.ProofRequirementReason
		)
		...
		if proofRequirement != proof.ProofRequirementReason_NOT_REQUIRED {
			...
			if !isProofFound {
				...
				claimExpiredEvent := tokenomics.EventClaimExpired{
					Claim:           &claim,
		...
		claimSettledEvent := tokenomics.EventClaimSettled{
			Claim:            &claim,
			NumRelays:        numRelaysInSessionTree,
			NumComputeUnits:  numClaimComputeUnits,
		...
		if err = ctx.EventManager().EmitTypedEvent(&proof.EventProofUpdated{
			Claim:           &claim,
			Proof:           nil,
			NumRelays:       0,
			NumComputeUnits: 0,
...
func (k Keeper) getExpiringClaims(ctx sdk.Context) (expiringClaims []proof.Claim, err error) {
	blockHeight := ctx.BlockHeight()
	...
	var nextKey []byte
	for {
		claimsRes, err := k.proofKeeper.AllClaims(ctx, &proof.QueryAllClaimsRequest{
			Pagination: &query.PageRequest{
				Key: nextKey,
			},
			Filter: &proof.QueryAllClaimsRequest_SessionEndHeight{
...
func (k Keeper) proofRequirementForClaim(ctx sdk.Context, claim *proof.Claim) (_ proof.ProofRequirementReason, err error) {
	logger := k.logger.With("method", "proofRequirementForClaim")

	var requirementReason = proof.ProofRequirementReason_NOT_REQUIRED
	...
	if numClaimComputeUnits >= proofParams.GetProofRequirementThreshold() {
		requirementReason = proof.ProofRequirementReason_THRESHOLD
	...
	if randFloat <= proofParams.GetProofRequestProbability() {
		requirementReason = proof.ProofRequirementReason_PROBABILISTIC

...
func (k msgServer) DelegateToGateway(ctx context.Context, msg *types.MsgDelegateToGateway) (*types.MsgDelegateToGatewayResponse, error) {
	...
	if !found {
		logger.Info(fmt.Sprintf("Application not found with address %q", msg.AppAddress))
		return nil, types.ErrAppNotFound.Wrapf(
			"application not found with address: %q",
			msg.AppAddress,
		)
	}
	logger.Info(fmt.Sprintf("Application found with address %q", msg.AppAddress))

	// Check if the gateway is staked
	if _, found := k.gatewayKeeper.GetGateway(ctx, msg.GatewayAddress); !found {
		logger.Info(fmt.Sprintf("Gateway not found with address %q", msg.GatewayAddress))
		return nil, types.ErrAppGatewayNotFound.Wrapf(
			"gateway not found with address: %q",
			msg.GatewayAddress,
		)
	}
	...
	if uint64(len(app.DelegateeGatewayAddresses)) >= maxDelegatedParam {
		logger.Info(fmt.Sprintf(
			"Application already delegated to maximum number of gateways: %d",
			maxDelegatedParam,
		))
		return nil, types.ErrAppMaxDelegatedGateways.Wrapf(
			"application already delegated to %d gateways",
			maxDelegatedParam,
		)
	}

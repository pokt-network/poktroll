...
func TestMsgServer_DelegateToGateway_SuccessfullyDelegate(t *testing.T) {
	...
	require.Equal(t, 1, len(foundApp.DelegateePubKeys))
	foundPubKey, err := types.AnyToPubKey(foundApp.DelegateePubKeys[0])
	require.NoError(t, err)
	...
	foundApp, isAppFound = k.GetApplication(ctx, appAddr)
	require.True(t, isAppFound)
	require.Equal(t, 2, len(foundApp.DelegateePubKeys))
	foundPubKey1, err := types.AnyToPubKey(foundApp.DelegateePubKeys[0])
	require.NoError(t, err)
	foundGatewayAddr1 := types.PublicKeyToAddress(foundPubKey1)
	require.Equal(t, gatewayAddr1, foundGatewayAddr1)
	foundPubKey2, err := types.AnyToPubKey(foundApp.DelegateePubKeys[1])
	require.NoError(t, err)

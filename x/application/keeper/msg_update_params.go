...
func (k msgServer) UpdateParams(goCtx context.Context, req *types.MsgUpdateParams) (*types.MsgUpdateParamsResponse, error) {
	...
	for _, paramsUpdate := range paramsUpdates {
		...
		if err := k.SetParamsUpdate(ctx, *paramsUpdate); err != nil {
			err = types.ErrAppParamInvalid.Wrapf("unable to set params deactivation height: %v", err)
	...
	// * The actual parameter update will be applied by BeginBlockerActivateApplicationParams
	...
	if err := k.SetParamsUpdate(ctx, paramsUpdate); err != nil {
		err = types.ErrAppParamInvalid.Wrapf("unable to set params: %v", err)
		logger.Error(err.Error())
		return nil, status.Error(codes.Internal, err.Error())
	}

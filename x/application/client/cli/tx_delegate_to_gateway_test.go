...
func TestCLI_DelegateToGateway(t *testing.T) {
	...
	tests := []struct {
		...
		{
			desc:           "delegate to gateway: valid",
			appAddress:     appAccount.Address.String(),
			gatewayAddress: gatewayAccount.Address.String(),
		},
		{
			desc: "delegate to gateway: invalid - missing app address",
			// appAddress:     appAccount.Address.String(),
			gatewayAddress: gatewayAccount.Address.String(),
			err:            types.ErrAppInvalidAddress,
		},
		{
			desc:           "delegate to gateway: invalid - invalid app address",
			appAddress:     "invalid address",
			gatewayAddress: gatewayAccount.Address.String(),
			err:            types.ErrAppInvalidAddress,
		},
		{
			desc:       "delegate to gateway: invalid - missing gateway address",
			appAddress: appAccount.Address.String(),
			// gatewayAddress: gatewayAccount.Address.String(),
			err: types.ErrAppInvalidGatewayAddress,
		},
		{
			desc:           "delegate to gateway: invalid - invalid gateway address",
			appAddress:     appAccount.Address.String(),

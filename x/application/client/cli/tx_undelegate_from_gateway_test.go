...
func TestCLI_UndelegateFromGateway(t *testing.T) {
	...
	tests := []struct {
		...
		{
			desc: "undelegate from gateway: invalid - missing app address",
			// appAddress:     appAccount.Address.String(),
			gatewayAddress: gatewayAccount.Address.String(),
			err:            types.ErrAppInvalidAddress,
		},
		{
			desc:           "undelegate from gateway: invalid - invalid app address",
			appAddress:     "invalid address",
			gatewayAddress: gatewayAccount.Address.String(),
			err:            types.ErrAppInvalidAddress,
		},
		{
			desc:       "undelegate from gateway: invalid - missing gateway address",
			appAddress: appAccount.Address.String(),
			// gatewayAddress: gatewayAccount.Address.String(),
			err: types.ErrAppInvalidGatewayAddress,
		},
		{
			desc:           "undelegate from gateway: invalid - invalid gateway address",
			appAddress:     appAccount.Address.String(),

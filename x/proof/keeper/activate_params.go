...
func (k Keeper) BeginBlockerActivateProofParams(
	...
	for _, proofParamsUpdate := range k.GetParamsUpdates(ctx) {
		...
		if proofParamsUpdate.EffectiveBlockHeight != uint64(currentBlockHeight) {
			continue
		}
		...
		eventParamsUpdated := &prooftypes.EventParamsUpdated{
			Params:               proofParamsUpdate.Params,
			EffectiveBlockHeight: proofParamsUpdate.EffectiveBlockHeight,
		}

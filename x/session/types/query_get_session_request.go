...
// NewQueryGetSessionRequest generates a new QueryGetSessionRequest object.
...
// ValidateBasic performs basic validation on this query request.
func (query *QueryGetSessionRequest) ValidateBasic() error {
	...
	if _, err := sdk.AccAddressFromBech32(query.ApplicationAddress); err != nil {
		return sdkerrors.Wrapf(
			ErrSessionInvalidAppAddress,
			"invalid app address for session being retrieved %s; (%v)", query.ApplicationAddress, err,
		)
	}
	...
	if !sharedhelpers.IsValidService(query.Service) {
		return sdkerrors.Wrapf(
			ErrSessionInvalidService,
			"invalid service for session being retrieved %s;", query.Service,
		)
	}
	...
	if query.BlockHeight < 0 { // Note that `0` defaults to the latest height rather than genesis
		return sdkerrors.Wrapf(
			ErrSessionInvalidBlockHeight,
			"invalid block height for session being retrieved %d;", query.BlockHeight,
		)
	}
	return nil
}

...
// CmdValidateCard returns a command that checks a service card offline.
//
...
func CmdValidateCard() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Validate a service card against the Pocket Service Card schema",
		Long: `Validate a service card file offline, before spending gas to publish it.
...
See docs/pocket_service_card.md for the schema.`,
		Example: `  pocketd tx service validate-card ./card.json`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(cmd.OutOrStdout(), "✅ valid service card: %s\n", cards.Summary(card))
			return nil
		},
	}

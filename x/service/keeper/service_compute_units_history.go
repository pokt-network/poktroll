...
func (k Keeper) SnapshotServiceComputeUnitsPerRelayChange(
	...
	if !k.hasServiceComputeUnitsPerRelayHistory(ctx, serviceId) {
...
// hasServiceComputeUnitsPerRelayHistory reports whether a service has any cupr history
// entry, without materializing them.
//
// SnapshotServiceComputeUnitsPerRelayChange runs on every cupr-changing MsgAddService —
// including bulk txs carrying dozens of service updates — and only needs to know whether
// the history is empty. Decoding every entry just to check len() == 0 grows with the
// service's update count for no reason.
func (k Keeper) hasServiceComputeUnitsPerRelayHistory(ctx context.Context, serviceId string) bool {
	store := runtime.KVStoreAdapter(k.storeService.OpenKVStore(ctx))
	serviceHistoryPrefix := types.ServiceComputeUnitsPerRelayHistoryKeyPrefixForService(serviceId)
	historyStore := prefix.NewStore(store, serviceHistoryPrefix)

...
type Service struct {
	...
	// Optional metadata carrying the service's card (see docs/pocket_service_card.md).
	// When exposed via JSON, the card is base64 encoded and MUST be <= 256 KiB when decoded.
...
// Metadata is a generic container for an onchain actor's descriptor, or "card".
// The raw bytes are stored onchain and will be base64 encoded in JSON representations.
//
// Next free index: 2
type Metadata struct {
	// A card: a small, self-describing JSON document.
	// Limited to 256 KiB when decoded. See docs/pocket_service_card.md for the schema.
	//
	// The chain deliberately does NOT validate the content of this payload beyond its
	// size. The card schema is versioned in-band (via its `schema` key) so it can evolve
	// without a consensus change; typed proto fields would freeze it at v1.
	//
	// A full method-level API specification does NOT belong here -- those routinely
	// exceed this cap by an order of magnitude. Point at one via the card's `specs[]`.
	//
	// TODO_FUTURE: promote settled card fields to dedicated proto fields once the schema
	// has stabilized AND something consumes it at runtime (e.g. QoS validating supplier
	// responses against the declared contract).
...
var fileDescriptor_4dfdeb4ae793ca69 = []byte{
	...
	,
}
...
func (m *Metadata) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.Card) > 0 {
		i -= len(m.Card)
		copy(dAtA[i:], m.Card)
		i = encodeVarintService(dAtA, i, uint64(len(m.Card)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil

...
func GetSessionStartHeight(sharedParamsUpdates []*ParamsUpdate, queryHeight int64) int64 {
	...
	sharedParamsUpdate := GetEffectiveParamsUpdate(sharedParamsUpdates, queryHeight)
	...
	paramsUpdateFirstSessionStartHeight := int64(sharedParamsUpdate.EffectiveBlockHeight)
...
func GetSessionNumber(sharedParamsUpdates []*ParamsUpdate, queryHeight int64) int64 {
	...
	for i, update := range sharedParamsUpdates {
		updateHeight := int64(update.EffectiveBlockHeight)
	...
	lastUpdate := GetEffectiveParamsUpdate(sharedParamsUpdates, queryHeight)
...
func GetEarliestSupplierProofCommitHeight(
	...
	proofWindowOpenHeight := GetProofWindowOpenHeight(sharedParamsUpdates, queryHeight)
...
type paramUpdate[T any] interface {
	GetParams() T
	GetEffectiveBlockHeight() uint64
}

// GetEffectiveParamsUpdate returns the effective params update as of the query height.
func GetEffectiveParamsUpdate[V any, T paramUpdate[V]](sharedParamsUpdates []T, queryHeight int64) T {
	...
	for _, update := range sharedParamsUpdates {
		...
		if update.GetEffectiveBlockHeight() > uint64(queryHeight) {
			break
		}

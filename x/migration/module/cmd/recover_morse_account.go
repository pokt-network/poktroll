...
func RecoverMorseAccountCmd() *cobra.Command {
	recoverCmd := &cobra.Command{
		...
		Short: "Recover a morse account which is unclaimable and on the recoverable accounts allowlist",
		Long: `Recover a morse account which is unclaimable and on the recoverable accounts allowlist.

The morse account recovery process is an authority-gated operation which may be invoked via authz or governance proposal.
This CLI command utilizes authz and therefore requires that an authorization exist onchain for the pocket.migration.MsgRecoverMorseAccount message, where the grantee is the message signer (the --from account).

To check which authz authorizations exists, see: pocketd query authz -h.`,
		Example: `# Recover the dao module account by Shannon destination key name

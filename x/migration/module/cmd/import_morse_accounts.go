...
func ImportMorseAccountsCmd() *cobra.Command {
	importMorseAcctsCmd := &cobra.Command{
		Use:   "import-morse-accounts [morse-account-state-json-path] [expected-morse-account-state-hash-base64]",
...
The import message will be broadcast ONLY if the given expected-morse-account-state-hash-base64 matches the actual base64-encoded canonical hash of the Morse account state loaded from morse-account-state-json-path.
This hash can be generated by running:
	$ pocketd tx migration hash-morse-accounts ./morse_account_state.json
...
func runImportMorseAccounts(cmd *cobra.Command, args []string) error {
	...
	expectedMorseAccountStateHashBase64 := args[1]
	actualMorseAccountStateHash, err := morseAccountState.GetHash()
	if err != nil {
		return err
	}
	actualMorseAccountStateHashBase64 := base64.StdEncoding.EncodeToString(actualMorseAccountStateHash)

	if expectedMorseAccountStateHashBase64 != actualMorseAccountStateHashBase64 {

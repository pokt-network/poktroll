...
func ImportMorseAccountsCmd() *cobra.Command {
	importMorseAcctsCmd := &cobra.Command{
		...
		Long: fmt.Sprintf(`Import the Morse accounts contained within the Morse account state JSON file.
The Morse account state JSON can be generated by transforming the "Morse state export".
See: %[1]spocket util export-genesis-for-reset -h%[1]s and %[1]spoktrolld tx migration collect-morse-accounts%[1]s.
...
// - Construct a MsgImportMorseClaimableAccounts from the authority and morse-account-state.
// - Sign and broadcast the MsgImportMorseClaimableAccounts using the Shannon key named by the `--from` flag.

...
var (
	flagUpdateHashOnly = "update-hash-only"
	updateHashOnly     bool
)
...
func ImportMorseAccountsCmd() *cobra.Command {
	importMorseAcctsCmd := &cobra.Command{
		...
		Long: `Import the Morse accounts contained within the given MsgImportMorseClaimableAccount JSON file.
...
func runImportMorseAccounts(cmd *cobra.Command, args []string) error {
	...
	if updateHashOnly {
		...
		if err = os.WriteFile(msgImportMorseClaimableAccountsJSONPath, msgImportMorseClaimableAccountsJSONBz, 0644); err != nil {
			return err
		}

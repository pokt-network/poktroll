...
func (pms PublicKeyMultiSignature) VerifySignature(msg []byte, sig []byte) bool {
	return pms.VerifyBytes(msg, sig)
}
...
func (pms PublicKeyMultiSignature) Type() string {
	return "multisig"
}
...
func (pms PublicKeyMultiSignature) Equals(other crypto.PubKey) bool {
	...
	for i := 0; i < len(pms.PublicKeys); i++ {
		if !pms.PublicKeys[i].Equals(otherKey.PublicKeys[i].PubKey()) {
			return false
		}
	}
	return true

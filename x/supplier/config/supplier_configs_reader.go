...
type YAMLStakeConfig struct {
	...
	DefaultRevSharePercent map[string]float64  `yaml:"default_rev_share_percent"`
}
...
type YAMLStakeService struct {
	...
	RevSharePercent map[string]float64    `yaml:"rev_share_percent"`
...
func ParseSupplierConfigs(ctx context.Context, configContent []byte) (*SupplierStakeConfig, error) {
	...
	defaultRevSharePercent := map[string]float64{}

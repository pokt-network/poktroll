...
- **A missed proof-path seed block no longer gets the supplier slashed.** `waitForBlock` returned the first block at height **>= target**, so if the miner did not observe the exact block it silently used a later one. That block's hash seeds the proof path, which the chain derives independently — so the submitted proof failed with an opaque path mismatch, the claim expired `PROOF_INVALID`, and the supplier was slashed for the whole session. It now detects the overshoot, warns, and queries the exact height. The exposure is worst when `proof_window_open_offset_blocks` is small, since the seed block sits immediately at the window opening.

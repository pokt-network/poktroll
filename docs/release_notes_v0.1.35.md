...
**Idempotent writes:** submitting a card byte-identical to the stored one — or omitting it — succeeds but writes nothing and emits no event, so repeated submissions do not grow state.
...
- **Proof-path seed block resolution hardened.** The miner now resolves the exact block it needs rather than accepting a later one, and warns if it has to fall back to a direct query. Previously a valid proof could be rejected. Exposure is larger when `proof_window_open_offset_blocks` is small.
...
- **A claim is no longer settled before its own proof window closes.** Ripeness is checked per claim against the params at that claim's own session end; a not-yet-ripe claim is deferred, not dropped.
...
- **Go 1.25.8 → 1.26.5**, alongside a `shannon-sdk` bump that raises its minimum. CI workflows and `Dockerfile.dev` are pinned to match.
- **`ring-go` → v0.2.0.** Worth calling out because ring signatures are consensus-reachable: `x/proof` verifies the relay request signature during proof validation. The bump is **behaviourally neutral onchain** — `Verify` and `hashToCurve` are byte-identical, and `challenge` is a pure allocation change producing the same bytes into the same hash. Every functional change is confined to the *signing* path (`SignerContext` / `SignWithContext`), which the chain never executes; plain `Sign` deliberately retains the un-optimised profile.
...
- ⚠️ **Bulk tokenomics params updates must include every field.** `MsgUpdateParams` (plural) replaces the whole struct — omitting `overservicing_bonus_multiplier` decodes it to `0`, which is coerced to `1`, silently reverting redistribution to OFF. The `bulk_params_main` and `bulk_params_beta` files are backfilled from live chain state; **`bulk_params_alpha` is NOT verified** (its RPC/gRPC endpoint no longer resolves) — re-derive it from the chain before ever using it.

  The accompanying tests check that every field is **present** and that the result passes `ValidateBasic`. They cannot detect a value that is merely **stale**, since that needs the live chain. Always diff a bulk file against `pocketd query <module> params` for the target network immediately before submitting it — a stale `num_blocks_per_session` in particular is a session-length change, and derived quantities move as `ceil()` step functions rather than proportionally.

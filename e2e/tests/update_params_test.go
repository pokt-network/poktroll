...
// start height then asserts that all module params have been updated as expected.
func (s *suite) AllModuleParamsShouldBeUpdatedAtTheNextSessionStart(moduleName string) {
	...
	s.assertExpectedModuleParamsUpdated(moduleName, uint64(nextSessionStart))
}
...
func (s *suite) assertExpectedModuleParamsUpdated(moduleName string, effectiveBlockHeight uint64) {
	s.Helper()
	...
	case tokenomicstypes.ModuleName:
		assertUpdatedParams(s,
			...
			&tokenomicstypes.QueryParamsResponse{
				...
				EffectiveBlockHeight: effectiveBlockHeight,
			},
		)
	case prooftypes.ModuleName:

...
Feature: Tokenomics Namespace
    ...
    Scenario: Single validator receives 100% of validator rewards (edge case)
        # Baseline
        Given the user has the pocketd binary installed
        ...
        # Configure tokenomics parameters for simplified distribution
        And the "tokenomics" module parameters are set as follows
            | name                                             | value | type  |
            | dao_reward_address                               | pokt1eeeksh2tvkh7wzmfrljnhw4wrhs55lcuvmekkw | string |
            | mint_allocation_percentages.dao                  | 0.0   | float |
            | mint_allocation_percentages.proposer             | 1.0   | float |
            | mint_allocation_percentages.supplier             | 0.0   | float |
            | mint_allocation_percentages.source_owner         | 0.0   | float |
            | mint_allocation_percentages.application          | 0.0   | float |
            | global_inflation_per_claim                       | 0     | float |
            | mint_equals_burn_claim_distribution.dao          | 0.0   | float |
            | mint_equals_burn_claim_distribution.proposer     | 1.0   | float |
            | mint_equals_burn_claim_distribution.supplier     | 0.0   | float |
            | mint_equals_burn_claim_distribution.source_owner | 0.0   | float |
            | mint_equals_burn_claim_distribution.application  | 0.0   | float |
        And all "tokenomics" module params should be updated
        ...
        And the "proof" module parameters are set as follows
            | name                         | value | type  |
            | proof_request_probability    | 0     | float |
            | proof_requirement_threshold  | 99999 | coin  |
            | proof_missing_penalty        | 32    | coin  |
            | proof_submission_fee         | 10    | coin  |
        And all "proof" module params should be updated

        # Start servicing relays
        When the supplier "supplier1" has serviced a session with "3" relays for service "anvil" for application "app1"

        # Wait for the Claim lifecycle
        And the user should wait for the "proof" module "CreateClaim" Message to be submitted

        # Record initial validator balance total BEFORE settlement
        And the user remembers the total validator balances as "validators_initial_balance"

        # Wait for settlement to complete
        And the user should wait for the ClaimSettled event with "NOT_REQUIRED" proof requirement to be broadcast

        # Validate the single validator edge case
        #
        # Total settlement: 12600 uPOKT = 3 relays * 100 compute units * 42 multiplier
        # Since we have 100% allocation to validators and only one validator,
        # that single validator should receive the full 12600 uPOKT

        # All validators (single validator) should receive 100% of the settlement amount
        Then the total validator balances should be "12600" uPOKT "more" than "validators_initial_balance"

        # The application stake should decrease by the full settlement amount
        And the "application" stake of "app1" should be "12600" uPOKT "less" than before
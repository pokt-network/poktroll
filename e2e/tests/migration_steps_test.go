...
func (s *migrationSuite) TheAuthorityExecutes(commandStr string) {
	...
	if len(commandStringParts) < 1 || commandStringParts[0] != "poktrolld" {
		s.Fatalf("ERROR: expected a pocketd command but got %q", commandStr)
	}
...
func (s *migrationSuite) TheShannonAccountIsFundedWith(fundCoinString string) {
	...
	require.Truef(s, isFound, "key %q not found in pocketd keyring", shannonKeyName)
...
func (s *migrationSuite) TheShannonDestinationAccountDoesNotExistOnchain() {
	...
	require.Truef(s, isFound, "key %q not found in pocketd keyring", s.getShannonKeyName())
...
func (s *migrationSuite) TheMorsePrivateKeyIsUsedToClaimAMorseclaimableaccountAsANonactorAccount() {
	...
	// pocketd tx migration claim-account --from=shannon-key-xxx <morse_src_address>
...
// is present in the pocketd keyring. If it is, it returns the address and true. Otherwise,

...
func (s *suite) newSharedMsgUpdateParams(params paramsMap) cosmostypes.Msg {
	...
	for paramName, paramValue := range params {
		...
		case sharedtypes.ParamApplicationTransferAndUnbondingPeriodSessions:
			msgUpdateParams.Params.ApplicationTransferAndUnbondingPeriodSessions = uint64(paramValue.value.(int64))
		default:
			s.Fatalf("ERROR: unexpected %q type param name %q", paramValue.typeStr, paramName)
		}
	}

...
#
#    # Wait for interval backup to ensure session state is persisted
#    And the user should wait for "12" seconds
#
#    # Non-gracefully restart the relay miner before claiming (interval backup should have captured state)
...
#    And the "application" stake of "app1" should be "23100" uPOKT "less" than before
  ...
  Scenario: Relay miner restores from backup after non-graceful shutdown between claiming and proving
    ...
    # Configure shared parameters with extended session and claim/proof windows for backup/restore testing
    And the "shared" module parameters are set as follows
      | compute_units_to_tokens_multiplier | 42 | int64 |
      | num_blocks_per_session            | 25 | int64 |
    ...
    # Wait a moment to ensure backup operations complete
    And the user should wait for "5" seconds

    # Non-gracefully restart the relay miner after claiming but before proving
    And the user gracefully restarts the relay miner "relayminer1"
    And the relay miner should restore from backup
    And the relay miner should continue from backup state
    ...
    # The application stake should be less 21000 * (1 + global_inflation) = 21000 * 1.1 = 23100
    And the "application" stake of "app1" should be "23100" uPOKT "less" than before
  Scenario: Relay miner performs graceful shutdown backup and successfully restores
    # Baseline setup
    Given the user has the pocketd binary installed
    ...
    # Service relays to have active sessions
    When the supplier "supplier1" has serviced a session with "5" relays for service "anvil" for application "app1"

    # Perform graceful shutdown to trigger backup
    And the user gracefully restarts the relay miner "relayminer1"
    And the relay miner should restore from backup
    And the relay miner should continue from backup state

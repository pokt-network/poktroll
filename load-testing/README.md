...
- [How to run tests](#how-to-run-tests)
  - [Adjust Virtual Users and Duration](#adjust-virtual-users-and-duration)
...
- (For local suite execution) LocalNet

## How to run tests

Tests are stored in the [load-testing/tests](./tests) folder, covering various use cases.
...
Default configurations for VU (virtual users) and duration are in the [config](./config/index.js) file. Override these by passing `--vus` and `--duration` flags to the `k6` command. Example:

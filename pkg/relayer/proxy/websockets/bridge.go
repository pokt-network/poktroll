...
func (b *bridge) handleGatewayIncomingMessage(msg message) {
	...
	if shouldRateLimit := b.relayMeter.ShouldRateLimit(b.ctx, relayRequest.Meta); shouldRateLimit {
		b.serviceBackendConn.handleError(
...
func (b *bridge) handleServiceBackendIncomingMessage(msg message) {
	...
	// Check if the relay should be rate-limited.
	// Recall that num inbound messages is unlikely to equal num outbound messages in a websocket.
	if shouldRateLimit := b.relayMeter.ShouldRateLimit(b.ctx, b.latestRelayRequest.Meta); shouldRateLimit {
		b.serviceBackendConn.handleError(
			ErrWebsocketsGatewayMessage.Wrapf("offchain rate limit hit by relayer proxy"),
		)
		return

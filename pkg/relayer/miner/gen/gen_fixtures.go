...
// TestFixtureGeneration_MineMockRelays is a test which generates relay fixtures
// for testing. It is not intended to be used **in/by** any tests but rather is
// persisted to aid in re-generation of relay fixtures should the test requirements
// change.
//

// This is utility for generating relay fixtures for testing. Output file is
...
// It returns an observable of mined relays which are published as they are mined, where difficulty is the difficulty threshold
...
func genRandomizedMinedRelayFixtures(
	...
	go func() {
		for {
			...
			// Hash relay bytes
			relayHash, err := hashRelay(newHasher, relayBz)
			if err != nil {
				errCh <- err
				return
			}
...
func hashRelay(newHasher func() hash.Hash, relayBz []byte) ([]byte, error) {
	hasher := newHasher()

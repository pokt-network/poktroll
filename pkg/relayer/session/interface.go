...
// and retrieval of SMSTs for a given session.
type RelayerSessions interface {
	// ClosingSessions returns an observable that notifies about sessions ready to be claimed.
	...
	// it creates a new SMST for it before returning it. An error is returned if the corresponding KVStore for SMST fails to be created.
...
type SessionTree interface {
	...
	// resources while waiting for the proof submission window to open.
	// Close gets the root hash of the SMST needed for submitting the claim; then commits the entire tree and stops the KVStore.
	// It should be called before submitting the claim on-chain. This function frees up the resources used by the SMST
	// that are no longer needed while waiting for the proof submission window to open.
	Close() (SMSTRoot []byte, err error)

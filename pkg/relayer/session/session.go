...
// SnapshotSessionTrees returns a point-in-time view of all session trees being
// tracked by the relayer sessions manager. The snapshot is created while
// holding the internal sessionsTreesMu lock to ensure the underlying map is not
// accessed concurrently. The resulting slice can be safely used by callers
// without additional synchronization.
func (rs *relayerSessionsManager) SnapshotSessionTrees() []relayer.SessionTreeSnapshot {
	rs.sessionsTreesMu.Lock()
	defer rs.sessionsTreesMu.Unlock()

...
func runRelay(cmd *cobra.Command, args []string) error {
	...
	for i := 1; i <= flagRelayRequestCount; i++ {
		...
		logger.Debug().Str("contentType", contentType).Msg("🔍 Backend response header")
		...
		if isStreaming {
			logger.Warn().Msg("⚠️ This is a streaming response, if more than one relay was requested the log data can be inconsistent.")

			logger.Debug().Msg("🌊 Handling streaming response.")

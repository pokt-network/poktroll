...
for such operations.`,
	...
	cmd.Flags().
		String(
			cosmosflags.FlagKeyringBackend,
			"",
	...
	cmd.Flags().
		StringVar(
			...
			omittedDefaultFlagValue,
			"Register the default Cosmos node flag, which is needed to initialize the "+
				"Cosmos query and tx contexts correctly. It can be used to override the "+
				"`QueryNodeRPCURL` and `TxNodeRPCURL` fields in the config file if specified.",
		)
	cmd.Flags().StringVar(
		&flagNodeGRPCURL,
		cosmosflags.FlagGRPC,
		omittedDefaultFlagValue,
		"Register the default Cosmos node grpc flag, which is needed to initialize the"+
			"Cosmos query context with grpc correctly. It can be used to override the "+
			"`QueryNodeGRPCURL` field in the config file if specified.",
	)
	cmd.Flags().
		Bool(
			cosmosflags.FlagGRPCInsecure,
			true,
			"Used to initialize the Cosmos query context with grpc security options."+
				"It can be used to override the `QueryNodeGRPCInsecure` field in the "+

...
const (
	...
	// maxTxRebroadcastsPerBlock bounds how many re-broadcasts are dispatched for a
	// single committed block.
	//
	// A whole claim/proof batch shares a submit and timeout height, so a large
	// supplier fleet can make thousands of txs due within the same few blocks. Without
	// a cap, one wave issues thousands of BroadcastTx round-trips, which (before this
	// cap existed) ran on the committed-block goroutine and could outlast a block —
	// delaying timeout processing and risking dropped blocks, whose txTimeoutPool
	// entries are then never processed at all.
	//
	// Txs over the cap are NOT dropped: their re-broadcast counter is only incremented
	// when they are actually collected, so an uncollected tx stays due and is picked up
	// on a subsequent block. Map iteration order is random, so the spillover rotates
	// across txs instead of starving the same ones every block.
	maxTxRebroadcastsPerBlock = 128

	// txRebroadcastConcurrency is the number of concurrent BroadcastTx calls used to
	// drain a re-broadcast wave. Keeps a wave short in wall-clock terms without
	// opening an unbounded number of connections to the node.
	txRebroadcastConcurrency = 8
)
...
type txClient struct {
	...
	// rebroadcastInFlight is true while a re-broadcast wave is still draining.
	// A wave runs off the committed-block goroutine, so this prevents a later block
	// from stacking a second wave on top of one that is still in progress. Skipping
	// loses nothing: uncollected txs keep their re-broadcast counter and become due
	// again on a subsequent block.
	rebroadcastInFlight atomic.Bool
...
func (txnClient *txClient) goTimeoutPendingTransactions(ctx context.Context) {
	...
	for block := range blockCh {
		...
		// Non-blocking: the wave drains on its own goroutine so a large batch can
		// never delay this loop's timeout processing (see rebroadcastDuePendingTxs).
		txnClient.rebroadcastDuePendingTxs(ctx, currentHeight)
...
// maxTxRebroadcasts times, spread across the window (the k-th resend is due at
// submitHeight + window*k/(maxTxRebroadcasts+1)), and never within
...
//
// The due set is collected synchronously (a fast, locked map scan) but BROADCAST
// ASYNCHRONOUSLY on a bounded worker pool. Broadcasting inline would put thousands
// of RPC round-trips on the committed-block goroutine for a large supplier fleet,
// delaying that height's timeout processing and risking dropped blocks — whose
// txTimeoutPool entries are then never processed and whose error channels never
// close. At most one wave is in flight at a time; see rebroadcastInFlight.
func (txnClient *txClient) rebroadcastDuePendingTxs(ctx context.Context, currentHeight int64) {
	// A previous wave is still draining. Skip this height rather than stacking a
	// second wave: uncollected txs keep their counter and become due again later.
	if !txnClient.rebroadcastInFlight.CompareAndSwap(false, true) {
		txnClient.logger.Warn().Msgf(
			"[TX] skipping re-broadcast at height %d: previous wave still draining",
			currentHeight,
		)
		return
	}

	due := txnClient.collectDueRebroadcasts(currentHeight)
	if len(due) == 0 {
		txnClient.rebroadcastInFlight.Store(false)
		return
	}

	go func() {
		defer txnClient.rebroadcastInFlight.Store(false)
		txnClient.drainRebroadcastWave(ctx, due, currentHeight)
	}()
}

// drainRebroadcastWave broadcasts a collected set of due txs using at most
// txRebroadcastConcurrency concurrent BroadcastTx calls, returning once the wave is
// drained or ctx is done.
func (txnClient *txClient) drainRebroadcastWave(
	ctx context.Context,
	due []rebroadcastItem,
	currentHeight int64,
) {
	concurrency := txRebroadcastConcurrency
	if len(due) < concurrency {
		concurrency = len(due)
	}

	itemCh := make(chan rebroadcastItem)
	...
	for i := 0; i < concurrency; i++ {
		go func() {
			...
			for item := range itemCh {
				...
				// Re-broadcasting identical bytes is idempotent: CometBFT dedups by
				// hash, so a "tx already exists in cache" response means the original
				// is still pending and is safe to ignore.
...
// (network-bound) BroadcastTx happens outside the lock, and off the committed-block
// goroutine, in drainRebroadcastWave.
//
// It also prunes entries whose timeout height has already passed: it is the only
// place guaranteed to run on every observed block, so it is the reliable point to
// bound the pool (see the deferredCount comment below and the prune below).
func (txnClient *txClient) collectDueRebroadcasts(currentHeight int64) []rebroadcastItem {
	...
	for hash, pending := range txnClient.rebroadcastPool {
		// Prune expired entries. goTimeoutPendingTransactions removes a tx when it
		// processes that tx's EXACT timeout height, so a height that is never
		// delivered (dropped block, restart, slow consumer) would otherwise leak the
		// entry for the lifetime of the process AND keep it in this per-block scan.
		// Deleting during range is safe in Go.
		if currentHeight >= pending.timeoutHeight {
			delete(txnClient.rebroadcastPool, hash)
			continue
		}
		if pending.rebroadcasts >= maxTxRebroadcasts {
			continue
		}
		// Bound the wave. Over-cap txs are deferred, not dropped: their counter is
		// only incremented when collected, so they stay due and are picked up on a
		// later block (map order is random, so the spillover rotates).
		if len(due) >= maxTxRebroadcastsPerBlock {
			deferredCount++
			continue
		}
		...
		// A window too short to hold maxTxRebroadcasts+1 distinct slots makes the
		// integer division collapse every due height onto (or near) submitHeight,
		// firing all re-broadcasts on the blocks immediately after the original — the
		// duplicate flood this schedule exists to avoid, and with no time for the
		// original to land first. Skip re-broadcasting entirely in that case: there is
		// no room for it to help.
		if window < int64(maxTxRebroadcasts+2) {
			continue
		}
	...
	if deferredCount > 0 {
		txnClient.logger.Warn().Msgf(
			"[TX] re-broadcast wave at height %d capped at %d txs; %d deferred to a later block",
			currentHeight, maxTxRebroadcastsPerBlock, deferredCount,
		)
	}

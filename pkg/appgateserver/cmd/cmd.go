...
2. The key associated with the 'signing_key' configuration directive belongs
   to the address provided in the request, otherwise the ring signature will not be valid.`,
	...
	cmd.Flags().Bool(cosmosflags.FlagGRPCInsecure, false, "Used to initialize the Cosmos query context with grpc security options. It can be used to override the `QueryNodeGRPCInsecure` field in the config file if specified.")

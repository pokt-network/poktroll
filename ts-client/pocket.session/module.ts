...
import { MsgUpdateParams } from "./types/pocket/session/tx";
import { MsgUpdateParam } from "./types/pocket/session/tx";

import { Params as typeParams} from "./types"
import { SessionHeader as typeSessionHeader} from "./types"
...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {
  ...
  return {
		...
		msgUpdateParams({ value }: msgUpdateParamsParams): EncodeObject {
			try {
				return { typeUrl: "/pocket.session.MsgUpdateParams", value: MsgUpdateParams.fromPartial( value ) }  
			} catch (e: any) {
		...
		msgUpdateParam({ value }: msgUpdateParamParams): EncodeObject {
			try {
				return { typeUrl: "/pocket.session.MsgUpdateParam", value: MsgUpdateParam.fromPartial( value ) }  
			} catch (e: any) {
...
class SDKModule {
	...
	constructor(client: IgniteClient) {		
		...
		this.structure =  {
						...
						SessionHeader: getStructure(typeSessionHeader.fromPartial({})),
...
const IgntModule = (test: IgniteClient) => {
	return {
		module: {
			PocketSession: new SDKModule(test)
		},
		registry: msgTypes
  }
}

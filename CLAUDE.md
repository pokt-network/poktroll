...
- **Offchain pricing must track the onchain epoch** - The RelayMiner recomputes claim value locally to decide whether a proof is worth submitting (`pkg/relayer/session/proof.go`). If it prices under different params than the chain, it can skip a proof the chain requires, which surfaces as `PROOF_MISSING` and slashes the supplier. Any change to how the chain resolves pricing params needs the matching change in `pkg/relayer/session`.
